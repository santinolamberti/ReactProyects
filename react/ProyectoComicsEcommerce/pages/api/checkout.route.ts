...
export default function handler(req: NextApiRequest, res: NextApiResponse<Data>) {
    ...
    try {
        ...
        if (body.usuario.entrega.direccion === invalidAddress) {
            res.status(400).json(ERROR_INCORRECT_ADDRESS);
            return
        }
        if (body.tarjeta.nroTarjeta === withoutFundsCard) {
            res.status(400).json(ERROR_CARD_WITHOUT_FUNDS);
            return
        }
        if (body.tarjeta.nroTarjeta === withoutAuthorizationCard) {
            res.status(400).json(ERROR_CARD_WITHOUT_AUTHORIZATION);
            return
        }
        if (body.tarjeta.nroTarjeta === validCard) {
